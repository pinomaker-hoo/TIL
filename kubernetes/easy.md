...
쿠버네티스 통신은 몇가지의 특징이 존재한다.

1. 파드가 사용하는 네트워크와 호스트(노드)가 사용하는 네트워크가 다르다.

   노드 내 파드들은 가상의 네트워크를 사용하고, 호스트는 물리 네트워크를 사용한다.

   <img src='https://github.com/pinomaker-hoo/TIL/assets/56928532/b33bcb69-171a-4bbd-b711-e223c743d802'  width=300 />

2. 같은 노드에 떠 있는 파드끼리만 통신이 가능하다.

   같은 노드에 떠있는 파드끼리는 통신이 가능하지만, 다른 노드의 파드나 외부와는 통신이 불가능하다.

3. 다른 노드와 통신을 할려면 CNI 플러그인이 필요하다.

   CNI 플로그인은 컨테이너들의 네트워크를 연결하거나 삭제하는 데 사용되는 컨테이너 간의 통신을 위한 네트워크 인터페이스 플러그인이다.

이제 파드 간의 통신에 대해서 학습을 정리한다.

1. 같은 파드에 포함된 컨테이너 간 통신

   같은 파드 내의 컨테이너끼리는 직접 로컬 호스트 통신이 가능하다. 하나의 파드에는 가상의 네트워크가 생성되며, 파드 내의 컨테이너들은 같은 가상 네트워크를 사용하기에 가능하다. 즉 하나의 파드 내의 컨테이너들은 모두 같은 IP를 사용한다.

   그러기에 컨테아너에 대한 구별은 포트번호를 사용하게된다.

   <img src='https://github.com/pinomaker-hoo/TIL/assets/56928532/c58443c8-9b79-4c66-8327-510ed0a7893e'  width=300 />

   <br />

2. 단일 노드에서 파드 간 통신

   단일 노드에 떠 있는 파드들은 같은 대역을 사용하기에 docker0 브리지를 통해 서로 통신이 가능하다

3. 다수의 노드에서 파드 간 통신

   파드에서 생성되는 가상 네트워크의 IP는 랜덤이기에 노드1의 파드와 노드2의 파드의 IP가 같은 경우가 야기될 수도 있다. 이러한 문제를 오버레이 네트워크를 통하여 해결할 수 있다.

   오버레이 네트워크란 노드에서 사용하는 물리적인 네트워크 위에 가상의 네트워크를 구성하는 것으로, 이를 이용하면 쿠버네티스 클러스로 묶인 모든 노드에 떠있는 파드 간의 통신이 가능해진다.

   <img src='https://github.com/pinomaker-hoo/TIL/assets/56928532/644f5a9b-dc2e-43e6-9b70-a8f076505785' width=600>
